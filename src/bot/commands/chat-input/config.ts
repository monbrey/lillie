...
export const execute = async (api: API, interaction: APIChatInputApplicationCommandInteraction) => {
	...
	if (!subcommand || subcommand.type !== ApplicationCommandOptionType.Subcommand)
		return api.interactions.reply(
			interaction.id,
			interaction.token,
			{ content: 'Error executing this command - no subcommand selected', flags: MessageFlags.Ephemeral },
		);
	...
	if (!subcommand.options) {
		return api.interactions.reply(
			interaction.id,
			interaction.token,
			{ content: 'Error executing this command - no configuration settings passed', flags: MessageFlags.Ephemeral },
		);
	}
	...
	try {
		...
		await api.interactions.reply(
			interaction.id,
			interaction.token,
			{ content: 'Configuration updated successfully', flags: MessageFlags.Ephemeral },
		)
	} catch (error) {
		await api.interactions.reply(
			interaction.id,
			interaction.token,
			{ content: 'Error updating configuration', flags: MessageFlags.Ephemeral },
		)
		console.error(error);
	}
}